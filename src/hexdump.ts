...
export function hexDumpLine(cur: Cursor, address: number): string {
  const lineLength = 16
  ...
  for (const value of buf) {
    line += value.toString(16).padStart(2, '0') as string + ' '
  }
  line += ' | '
...
export function hexDump(cur: Cursor, opt: {start: number, length: number, lineLength: number} = {start: 0, length: 0, lineLength: 16}): string {
