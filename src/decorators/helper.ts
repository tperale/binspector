...
import { TransformerExecAera, transformerDecoratorFactory } from './transformer';
...
export function Char<This extends object, Value> (_: undefined, context: Context<This, Value>): void {
  ...
  transformerDecoratorFactory('transform-char-inner-read', x => String.fromCharCode(x), { aera: TransformerExecAera.PrimitiveTranformer })(_, context)
  transformerDecoratorFactory('transform-char-inner-write', x => String(x).charCodeAt(0), { aera: TransformerExecAera.PrimitiveTranformer, scope: ExecutionScope.OnWrite })(_, context)
}
...
export function Ascii<This extends object, Value> (_: undefined, context: Context<This, Value>): void {
  ...
  transformerDecoratorFactory('transform-ascii-outer-read', x => Array.isArray(x) ? x.join('') : x)(_, context)
  transformerDecoratorFactory('transform-ascii-outer-write', x => x.length > 1 ? x.split('') : x, { scope: ExecutionScope.OnWrite })(_, context)
}
...
export function NullTerminatedString<This extends object, Value> (opt?: Partial<ControllerOptions>): DecoratorType<This, Value> {
  return function<This extends object, Value> (_: undefined, context: Context<This, Value>): void {
    ...
    transformerDecoratorFactory('transform-null-termination-read', x => x.slice(0, -1))(_, context)
    transformerDecoratorFactory('transform-null-termination-write', x => x + '\0', { scope: ExecutionScope.OnWrite })(_, context)
  }
}
